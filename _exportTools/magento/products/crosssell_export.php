...
class Product_Export
{
    ...
    public function run()
    {
        ...
        $fp = fopen('crosssell_export.csv', 'w');
    ...
    {	
        $upsell_skus = array();
        $upsell = $magento_product->getUpsellProductCollection();
        foreach($upsell as $upsell_product)
        {
            $upsell_skus[] = $upsell_product->getSku();
        }

        $related_skus = array();
        $related = $magento_product->getRelatedProductCollection();
        foreach($related as $related_product)
        {
            $related_skus[] = $related_product->getSku();
        }

        $crosssell_skus = array();
        $crosssell = $magento_product->getCrosssellProductCollection();
        foreach($crosssell as $crosssell_product)
        {
            $crosssell_skus[] = $crosssell_product->getSku();
        }
        ...
        $tmp = array(
            ...
            'upsell' => implode('|', $upsell_skus),
            'related' => implode('|', $related_skus),
            'crosssell' => implode('|', $crosssell_skus),
        );
    ...
    public function getMagentoProducts()
    {
        if(!$this->_magento_products)
        {
            ...
            $collection = $asdasd 
                ...
                //->addAttributeToFilter('sku', '829952') //group
